...
namespace Telegram.Messaging.Messaging
{
	...
	public class MessageManager
	{
		...
		readonly TelegramBotClient tClient;
		long _chatId;
		/// <summary>
		...
		/// </summary>
		public int TId => _tid != 0 ? _tid : CurrentMessage?.From?.Id ?? 0;
		...
		public MessageManager(string botName, string botToken, long chatId = 0, int userId = 0)
		...
		/// <param name="chatId">The chat id where this manager will address the messages. If it is left to its default value, bot will only be able to reply
		/// messages after an incoming message is processed</param>
		public MessageManager(string botName, string botToken, TelegramBotClient client, long chatId = 0, int userId = 0)
		{
		...
		public Action<MessagingEventArgs> GetDelegateOnTarget(Action<MessagingEventArgs> forAction)
		{
			...
			Action<MessagingEventArgs> toret = null;
			lock (syncHndlrs)
			{
				if (callBackhandlers.ContainsKey(skey))
				{
					// we already have a target for this action, we must create  a new delegate
					// that takes as target the object that we already have
					target = callBackhandlers[skey];
					toret = (Action<MessagingEventArgs>)Action.CreateDelegate(typeof(Action<MessagingEventArgs>), target, forAction.Method);
				}
				else
				{
					// we don't have a delegate for this action, we can add the target to our list of handlers
		...
		async Task RaiseOnCommand(CommandReceivedEventArgs e)
		{
			...
			if (e?.OriginatingQuestion?.OnEvent != null)
			{
				var onEvent = GetDelegateOnTarget(e.OriginatingQuestion.OnEvent);
		...
		async Task RaiseOnDiceRolled(DiceRolledEventArgs e)
		{
			...
			if (e?.OriginatingQuestion?.CallbackHandler != null)
			{
				...
				var t = Task.Run(() => callbackHandler.OnDiceRolled(this, e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
			if (e?.OriginatingQuestion?.OnEvent != null)
		...
		async Task RaiseOnQuestionAnswered(QuestionAnsweredEventArgs e)
		{
			...
			if (e?.AnsweredQuestion?.CallbackHandler != null)
			{
				...
				var t = Task.Run(() => callbackHandler.OnQuestionAnswered(this, e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
			if (e?.AnsweredQuestion?.OnEvent != null)
			{
				var onEvent = GetDelegateOnTarget(e.AnsweredQuestion.OnEvent);
				Task t = Task.Run(() => onEvent(e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
		...
		async Task RaiseOnQuestionChanged(QuestionChangedEventArgs e)
		{
			...
			if (e?.CurrentQuestion?.CallbackHandler != null)
			{
				...
				var t = Task.Run(() => callbackHandler.OnQuestionChanged(this, e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
			if (e?.CurrentQuestion?.OnEvent != null)
			{
				var onEvent = GetDelegateOnTarget(e.CurrentQuestion.OnEvent);
				Task t = Task.Run(() => onEvent(e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
		...
		async Task RaiseOnPageChanged(ChangePageEventArgs e)
		{
			...
			{
				var callbackHandler = GetHandler(e.CurrentQuestion.CallbackHandler) as QuestionAnswerCallbackHandler;
				var t = Task.Run(() => callbackHandler.OnPageChanged(this, e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
			if (e?.CurrentQuestion?.OnEvent != null)
		...
		async Task RaiseOnSurveyCancelled(SurveyCancelledEventArgs e)
		{
			...
			{
				var callbackHandler = GetHandler(e.CurrentQuestion.CallbackHandler) as QuestionAnswerCallbackHandler;
				var t = Task.Run(() => callbackHandler.OnSurveyCancelled(this, e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
			if (e?.CurrentQuestion?.OnEvent != null)
		...
		async Task RaiseOnSurveyCompleted(SurveyCompletedEventArgs e)
		{
			...
			if (e?.Survey?.MostRecentQuestion?.CallbackHandler != null)
			{
				...
				var t = Task.Run(() => callbackHandler.OnSurveyCompleted(this, e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
			if (e?.Survey?.MostRecentQuestion?.OnEvent != null)
			{
				var onEvent = GetDelegateOnTarget(e.Survey.MostRecentQuestion.OnEvent);
				Task t = Task.Run(() => onEvent(e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
		...
		async Task RaiseOnPayPressed(PayReceivedEventArgs e)
		{
			...
			{
				var callbackHandler = GetHandler(e.CurrentQuestion.CallbackHandler) as QuestionAnswerCallbackHandler;
				var t = Task.Run(() => callbackHandler.OnPayPressed(this, e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
			if (e?.CurrentQuestion?.OnEvent != null)
		...
		async Task RaiseOnInvalidInteraction(InvalidInteractionEventArgs e)
		{
			...
			{
				var callbackHandler = GetHandler(e.OriginatingQuestion.CallbackHandler) as QuestionAnswerCallbackHandler;
				var t = Task.Run(() => callbackHandler.OnInvalidInteraction(this, e));
				t.ConfigureAwait(false);
				tasks.Add(t);
			}
			if (e?.OriginatingQuestion?.OnEvent != null)
		...
		private async Task<TelegramMessage> ProcessCurrentMessage()
		{
			...
			try
			{
				...
				if (CurrentSurvey != null && // == null if the survey was just cancelled && 
					mostRecent.IsCompleted && CurrentSurvey.MostRecentQuestion == mostRecent)
		...
		static InlineKeyboardMarkup CreateInlineMarkupKeyboard(TelegramChoice[] choices, int maxButtPerRow = 0)
		{
			...
			foreach (TelegramChoice a in onlyChoices)
			{
				...
				currentRow.Add(new InlineKeyboardButton() { Text = a.Label, CallbackData = a.ToJsonSpecial(), Url = a.IsUrl ? a.Value : null });
			...
			foreach (TelegramChoice a in onlyCommands)
			{
				if (a.Equals(TelegramChoice.PayAnswer)) // pay must be the first
				{
					...
					currentRow.Insert(0, new InlineKeyboardButton() { Text = a.Label, Pay = true, CallbackData = a.ToJsonSpecial() });
					currentRow = null;
				}
				...
				if (currentRow == null) // only if there is a command which is not in the IFs
					currentRow = new List<InlineKeyboardButton>();
				currentRow.Add(new InlineKeyboardButton() { Text = a.Label, CallbackData = a.ToJsonSpecial() });
			}
		...
		/// <returns></returns>
		private static async Task<Message> doSendQuestion(TelegramBotClient tClient, MessageManager mngr, Question question, bool updating, bool isCallbackQuery)
		{
